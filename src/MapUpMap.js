...
const map =  
  ...
  async function find(value) {
    ...
    .then(function (response) {
        ...
        {
          let entry = place.results[0].GAZETTEER_ENTRY;
           let x = entry.GEOMETRY_X;
           let y = entry.GEOMETRY_Y;
           let coords = OStoLatLong(y, x);
           console.log("Result found at " + entry.NAME1 + ", " + entry.COUNTY_UNITARY + ", " + entry.COUNTRY);
           console.log(coords);
           map.flyTo(coords[1], coords[0])
        }
  ...
  function OStoLatLong(northing, easting) {
    proj4.defs("EPSG:27700", "+proj=tmerc +lat_0=49 +lon_0=-2 +k=0.9996012717 +x_0=400000 +y_0=-100000 +ellps=airy +datum=OSGB36 +units=m +no_defs");
    let coords = proj4('EPSG:27700','EPSG:4326').forward( [northing, easting] ).reverse();
    return coords;
  }
